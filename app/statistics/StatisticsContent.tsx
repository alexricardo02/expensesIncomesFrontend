      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 space-y-6">
          <div className="flex flex-wrap gap-2 items-center">
            ...
            <button onClick={() => handlePillClick("thisMonth")} className="px-4 py-2 bg-slate-100 hover:bg-slate-200 rounded-full text-xs font-semibold">{t("statistics.thisMonth")}</button>
            <button onClick={() => handlePillClick("lastMonth")} className="px-4 py-2 bg-slate-100 hover:bg-slate-200 rounded-full text-xs font-semibold">{t("statistics.lastMonth")}</button>
            <button onClick={() => handlePillClick("last3Months")} className="px-4 py-2 bg-slate-100 hover:bg-slate-200 rounded-full text-xs font-semibold">{t("statistics.last3Months")}</button>
            <button onClick={() => handlePillClick("thisYear")} className="px-4 py-2 bg-slate-100 hover:bg-slate-200 rounded-full text-xs font-semibold">{t("statistics.thisYear")}</button>
            <button onClick={() => updateFilter({ startDate: null, endDate: null })} className="px-4 py-2 bg-slate-100 hover:bg-slate-200 rounded-full text-xs font-semibold">{t("statistics.allTime")}</button>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 lg:col-span-2">
            <h3 className="text-lg font-semibold mb-6 flex items-center gap-2"><TrendingUp size={20} className="text-indigo-500" /> {t("statistics.accumulatedBalance")}</h3>
          ...
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 flex flex-col items-center">
            <h3 className="text-lg font-semibold mb-6">{t("statistics.expensesByCategory")}</h3>
          ...
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 flex flex-col items-center">
            <h3 className="text-lg font-semibold mb-6">{t("statistics.incomesByCategory")}</h3>
            <div className="w-full max-w-72">
          ...
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 lg:col-span-2 flex flex-col items-center">
            <h3 className="text-lg font-semibold mb-6">{t("statistics.expensesComparison")}</h3>
            <div className="w-full h-72">
                ...
                <tbody>
                  {data.transactions.map((tx: any, idx: number) => (
                    <tr key={idx} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors">
                      ...
                      <td className="py-3 pr-4 text-sm font-medium text-slate-700">{tx.description || t("common.na")}</td>
